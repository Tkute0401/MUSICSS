    ...
    public void onBindViewHolder(@NonNull SongAdapterViewHolder holder, int position) {
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                int adapterPosition = holder.getAdapterPosition(); // Use this to get the clicked item's position

                if (adapterPosition == RecyclerView.NO_POSITION) {
                    return; // Check if the item position is valid
                }

                if (m != null && m.isPlaying()) {
                    m.reset(); // Reset the MediaPlayer instance
                }

                MyMediaPlayer.currentIndex = adapterPosition; // Set the current index
